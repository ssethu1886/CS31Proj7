...
//  proj7
//
//  Created by Swetha S on 5/29/22.
//
...
void Cyborg::forceMove(int dir)
{
    ...
    switch(dir){
        //moves north
        case 0:
            //check if off the grid or into a wall
...
string Player::move(int dir)
{
    ...
    // player moves North
    if (dir == 0){
        //check if we move off the grid
        if (m_row-1 < 1){
            return "Player couldn't move; player stands.";
        }
        //check if we move into a wall
        if (m_arena->hasWallAt(m_row-1, m_col)){
            return "Player couldn't move; player stands.";
        }
        //check if we move onto a cyborg
        if (m_arena->numberOfCyborgsAt(m_row-1, m_col) > 0){
            m_dead = true;
    ...
    if (dir == 1){
        ...
        //check if we move into a wall
        if (m_arena->hasWallAt(m_row, m_col+1)){
            return "Player couldn't move; player stands.";
        }
        //check if we move onto a cyborg
        if (m_arena->numberOfCyborgsAt(m_row, m_col+1) > 0){
            m_dead = true;
            return "Player walked into a cyborg and died.";
        }
        //successful move
    ...
    // to avoid error of not returning in all control paths, but should never be executed
    return "Player couldn't move; player stands.";
}
...
void Arena::display(string msg) const
{
    ...
    for (int i = 0; i < m_nCyborgs; i++){
        displayGrid[m_cyborgs[i]->row()-1][m_cyborgs[i]->col()-1] = static_cast<char>('0' + m_cyborgs[i]->channel());
    }
...
string Arena::moveCyborgs(int channel, int dir)
{
    ...
    for (int i = 0; i<nCyborgsOriginally; i++){
        
        // those who are on channel and willRespond is true, move in dir
        if(m_cyborgs[i]->channel() == channel && willRespond){
       ...
       // if moves to player spot, mark player as dead
        if (m_cyborgs[i]->row() == m_player->row()
            && m_cyborgs[i]->col() == m_player->col()){
            m_player->setDead();
        }
        //if cyborg is dead, delete it
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
    ...
    // check you don't go off grid
    if (r-1 < 1 || a.hasWallAt(r-1, c)){
        northBorgs = 1000;
    }
    
    // check how many cyborgs are next to you in the north
  
    else{
    // above
    if (a.numberOfCyborgsAt(r-2, c) > 0)
        northBorgs++;
    // left
    if (a.numberOfCyborgsAt(r-1, c-1) > 0)
        northBorgs++;
    //right
    if (a.numberOfCyborgsAt(r-1, c+1) > 0)
        northBorgs++;
    ...
    // check how many cyborgs are next to you in east
    else{
    // above
    if (a.numberOfCyborgsAt(r-1, c+1) > 0)
        eastBorgs++;
    ...
    if (a.numberOfCyborgsAt(r+1, c+1) > 0)
        eastBorgs++;
    //right
    if (a.numberOfCyborgsAt(r, c+2) > 0)
        eastBorgs++;
    }
    ...
    // check how many cyborgs are next to you in south
    else{
    // below
    if (a.numberOfCyborgsAt(r+2, c) > 0)
        southBorgs++;
    ...
    // check how many cyborgs are next to you in west
    else{
    // above
    if (a.numberOfCyborgsAt(r-1, c-1) > 0)
        westBorgs++;
    ...
    // find how many cyborgs are next to you standing
    ...
    // find which move will put you next to min cyborgs
    int minBorgs = northBorgs;
    bestDir = 0;
    if (eastBorgs<minBorgs){
        minBorgs = eastBorgs;
        bestDir = 1;
    }
    if (southBorgs<minBorgs){
        minBorgs = southBorgs;
        bestDir = 2;
    }
    if (westBorgs<minBorgs){
        minBorgs = westBorgs;
        bestDir = 3;
    }
    if (standBorgs<minBorgs){
        minBorgs = standBorgs;
        return false;
    }
    return true;
